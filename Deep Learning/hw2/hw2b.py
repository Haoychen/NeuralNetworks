...
class myMLP(object):
    ...
    class).
    """
    def __init__(self, rng, input, n_in, n_hidden, n_out, n_hidden_layer, activation_function):
        """Initialize the parameters for the multilayer perceptron
        ...
        :param activation_function: a string used to specify the activation function: tanh or softmax

        """

        # Define the hidden layers, the input of the hidden layers after the first hidden layer
        # is the output of its last hidden layer
        self.hidden_layers = []
        for layer in range(n_hidden_layer):
            if layer == 0:
                if activation_function == 'tanh':
                    temp_hidden_layer = HiddenLayer(
                        rng=rng,
                        input=input,
                        n_in=n_in,
                        n_out=n_hidden,
                    ...
                    self.hidden_layers.append(temp_hidden_layer)
                else:
                    temp_hidden_layer = HiddenLayer(
                        rng=rng,
                        input=input,
                ...
                if activation_function == 'tanh':
                    temp_hidden_layer = HiddenLayer(
                        rng=rng,
                        input=self.hidden_layers[layer - 1].output,
                        n_in=n_hidden,
                        n_out=n_hidden,
                        activation=T.tanh
                    )
        ...
        l1_norm = 0
        for layer in range(n_hidden_layer):
            l1_norm = l1_norm + abs(self.hidden_layers[layer].W).sum()
        self.L1 = (l1_norm + abs(self.logRegressionLayer.W).sum())

        # square of L2 norm ; one regularization option is to enforce
        # square of L2 norm to be small
        l2_sqr = 0
        for layer in range(n_hidden_layer):
            l2_sqr = l2_sqr + (self.hidden_layers[layer].W ** 2).sum()
        self.L2_sqr = (l2_sqr + (self.logRegressionLayer.W ** 2).sum())
        ...
        # the parameters of the model are the parameters of
        # the multiple layer it is made out of
        params = self.hidden_layers[0].params
        for layer in range(1, n_hidden_layer):
            params += self.hidden_layers[layer].params
        self.params = params + self.logRegressionLayer.params
...
def test_mlp(learning_rate=0.01, L1_reg=0.00, L2_reg=0.0001, n_epochs=1000,
             batch_size=20, n_hidden=500, verbose=False, n_hidden_layer=1, activation_function='tanh'):
    """
