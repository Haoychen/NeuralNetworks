...
class ConvLayer(object):
    ...
    def __init__(self, rng, input, filter_shape, image_shape, stride=1, border_mode='valid', W=None, b=None):
        """
...
class DropoutConvLayer(object):
    ...
    def __init__(self, rng, input, filter_shape, image_shape, is_train, stride=1, p=0.5, border_mode='valid', W=None, b=None):
        """
            Allocate a LeNetConvPoolLayer with shared variable internal parameters.
        ...
        # self.output = T.nnet.relu(conv_out + self.b.dimshuffle('x', 0, 'x', 'x'))
        output = relu(conv_out + self.b.dimshuffle('x', 0, 'x', 'x'))
...
class batchNormLenetConvAvgPoolLayer(object):
    """Pool Layer of a convolutional network """
...
class DropoutPoolLayer(object):

    def __init__(self, input, ds, st, is_train, p=0.5):
        ...
        train_output = drop(pooled_out, 0.5)
        self.output = self.output = T.switch(T.neq(is_train, 0), train_output, p * pooled_out)
